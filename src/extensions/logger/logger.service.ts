...
export class LoggerService implements LoggerBase {
  ...
  log(message: string, ...meta: unknown[]): void {
    if (meta.length > 0) this.logger.info(this.buildContext(meta), message);
  ...
  warn(message: string, ...meta: unknown[]): void {
    if (meta.length > 0) this.logger.warn(this.buildContext(meta), message);
    else this.logger.warn(message);
  }

  debug(message: string, ...meta: unknown[]): void {
    if (meta.length > 0) this.logger.debug(this.buildContext(meta), message);
